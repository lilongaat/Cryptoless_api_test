...
class Test_create_account:
    ...
    test_data = [
        ("创建安全账户自动激活","ae0f28a2d98211ea6f656ecffa8a821235f78354921d63346c6be48a52610187",["0331e3ab5059c28098131d50856a99fcf40bea39b61f08ea55e1f35fbed131d2c0","02ac5e97a0659d6ce59c5b10b2ace331bffa276a555ff333b09beda99bd6b9f52e"],"ETH-RINKEBY","ETH-RINKEBY")
    ]
    ...
    def test_create_account_safe(self, test_title, privatekey, PublicKeys, networkCode, symbol):
        ...
        with allure.step("创建账户"):
            acc = Http.HttpUtils.post_account(networkCode,symbol,PublicKeys,2)
        ...
        with allure.step("查询激活账户交易 交易成功"):
            for i in range(10):
                sleep(10)
                act_ = Http.HttpUtils.post_safe_activation(accountid)
                assert act_.status_code == 200
                if (act_.json()["transaction"]["status"] == "SETTLED"):
                    assert act_.json()["account"]["status"] == 2
                    assert act_.json()["status"] == 1
                    break
