...
if __name__ == "__main__":
    ...
    # 固定时间运行
    scheduler.add_job(job_test_user, 'cron', hour=10, minute=20)
    scheduler.add_job(job_test_account, 'cron', hour=10, minute=30)
    scheduler.add_job(job_transfer_transfer, 'cron', hour=10, minute=40)
    scheduler.add_job(job_transfer_Swap, 'cron', hour=11, minute=00)
    scheduler.add_job(job_transfer_Stake, 'cron', hour=11, minute=10)
    scheduler.add_job(job_allure, 'cron', hour=11, minute=20)
    scheduler.start()
