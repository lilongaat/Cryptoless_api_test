...
from Common import Http_Enhancedapi
...
class Test_Enhanced_API_tokenAssets:
    ...
    test_data = [
        ...
        ("查询地址ERC20资产","0x56eddb7aa87536c09ccc2793473599fd21a8b17f","erc20",""),
        ("查询地址ERC20资产","0xb32Ffd383759Ab23C56e907DB8D33E498467847e","erc20",""),
        ("查询地址ERC20资产","0xB0253D0308125d7eD0AaE9f2Ff138C85CcFFcc07","erc20",""),
        ("查询地址ERC20资产","0x2D4b1580A0db58781a6D3A9d2CCbed16128FA0c9","erc20",""),
        ("查询地址ERC20资产","0x97B67E5D4eDA1a4A2A1687Fe59336eB4F7cFCec0","erc20",""),
        ("查询地址ERC721资产","0x3257eff4bdcb74b6ebf8f0f4c67f5d15288cf97c","erc721",""),
        ("查询地址ERC721资产","0x1e7067c402f89b3623d774dcda4a11e0d62bdabe","erc721",""),
        ("查询地址ERC721资产","0x00cdC153Aa8894D08207719Fe921FfF964f28Ba3","erc721",""),
        ("查询地址ERC721资产","0x6887246668a3b87F54DeB3b94Ba47a6f63F32985","erc721",""),
        ("查询地址ERC721资产","0x533e3c0e6b48010873B947bddC4721b1bDFF9648","erc721",""),
        ("查询地址ERC1155资产","0x477b8D5eF7C2C42DB84deB555419cd817c336b6F","erc1155",""),
        ("查询地址ERC1155资产","0xd88C972516714C6cb182c08cC4116dd3d5056f86","erc1155",""),
        ("查询地址ERC1155资产","0x8a677474dEC43FCAeB80a2Dd2B1bc5D945aB61d1","erc1155",""),
        ("查询地址ERC1155资产","0xa4f86E2f93376cF8DFB5de2f60CEA840df9E6eF0","erc1155",""),
        ("查询地址ERC1155资产","0x68b3465833fb72A70ecDF485E0e4C7bD8665Fc45","erc1155",""),
    ]
    ...
    def test_tokenassets(self, test_title,address,types,limit):
        ...
        with allure.step("Query tokenAssets!"):
        ...
        with allure.step("size 与 ethplorer-tokens 条数对比!"):
            if (res.json()["size"] == 0 and len(res.json()["data"]) == 0):
                assert 'tokens' not in res_ethplorer.json(),'Tokens Number Gaps!'
                logger.info("\n\n" + "<-----Tokens Null----->")         
            else:
                # assert res.json()["size"] == len(res.json()["data"]) == len(res_ethplorer.json()["tokens"]),'Tokens Number Gaps!'
                pytest_check.equal(len(res.json()["data"]),len(res_ethplorer.json()["tokens"]),'Tokens Number Gaps!')

                with allure.step("tokens 资产对比!"):
                    token_gaps = []
...
#     # pytest.main(["-vs", "/Users/lilong/Documents/Test_Api/Testcase/Test_Serve/Test_Enhanced_API/test_enhanced_api.py::Test_Enhanced_API_tokenInfo"])
#     pytest.main(["-vs", "/Users/lilong/Documents/Test_Api/Testcase/Test_Serve/Test_Enhanced_API/test_enhanced_api.py::Test_Enhanced_API_balance"])
